...
- **Database:** MariaDB 10.11 at `127.0.0.1:3306`, database `my_pos`, user `root`.
- **ORM:** Prisma 7.8.0 with `@prisma/adapter-mariadb`. Client output: `@/generated/prisma`. Schema: `prisma/schema.prisma`.
...
- **Store Settings:** `StoreSettings` in `@/types/pengaturan`. Per-device settings stored in `localStorage` via `@/lib/settings-client`, global defaults in MariaDB table `Pengaturan`.
